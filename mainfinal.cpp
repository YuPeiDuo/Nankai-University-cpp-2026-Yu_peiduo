...
public:
    Background(QWidget *parent = nullptr) : QWidget(parent)
    {
        ...
        close_page.load("C:/Users/39036/Downloads/关闭页面1.png");
        ...
        is_bought_hat[1] = false;
        is_bought_hat[2] = false;
        is_bought_hat[3] = false;
        ...
        minAngle = -30.0;
        maxAngle = 30.0;
        stopGameTimer.setSingleShot(false);
        connect(&stopGameTimer, &QTimer::timeout, this, &Background::updatePointer);
        shop_close.load("C:/Users/39036/Downloads/关闭页面1.png");
        shop_close1.load("C:/Users/39036/Downloads/关闭页面1.png");
        decoration_shop.load("C:/Users/39036/Downloads/decoration-shop-removebg-preview.jpeg");
        rabbithat.load("C:/Users/39036/Downloads/rabbithat-removebg-preview.png");
        dinosaurhat.load("C:/Users/39036/Downloads/dinosaurhat-removebg-preview.png");
        doghat.load("C:/Users/39036/Downloads/doghat-removebg-preview.png");
        picture.load("C:/Users/39036/Downloads/picture.jpeg");
        curtain_inshop.load("C:/Users/39036/Downloads/chuanglianclose-removebg-preview.png");
        flower_inshop.load("C:/Users/39036/Downloads/flower4-shadow-removebg-preview.png");
        picture_inshop.load("C:/Users/39036/Downloads/picture.jpeg");
        quizWin = false;
        rpsWin = false;
        rpsFail = false;
    }
...
private:
    ...
    bool is_bought_curtain = false;
    bool is_open_curtain = false;
    bool is_bought_flower = false;
    ...
    QPixmap doghat;
    QPixmap picture;
    QPixmap picture_inshop;
    QPixmap flower_inshop;
    QPixmap curtain_inshop;
    bool quizWin;
    bool rpsWin;
    bool rpsFail;
    void initPuzzle() {
    ...
    bool isPointOnCurtain(QPoint& pos)
    {
        ...
        if (isoncurtain.contains(pos) && !enter_quiz && !enter_shop && !enter_decoration) return true;
        return false;
    }
    ...
    bool isPointOnExit(QPoint& pos)
    {
        ...
        if (isonexit.contains(pos) && !enter_quiz && !enter_shop && !enter_decoration) return true;
        return false;
    }
    ...
    bool isPointOnDecoration(QPoint& pos)
    {
        ...
        if (isondec.contains(pos) && !enter_quiz && !enter_shop && !enter_decoration) return true;
        return false;
    }
    ...
    bool isPointOnShop6(QPoint& pos) {
        ...
        {
            QRect isexitshop (2460,0,close_page.width(),close_page.height());
        ...
        bool isPointOnDesc(QPoint& pos)
        {
            ...
            if (desc_rect.contains(pos)  && !enter_quiz && !enter_shop && !enter_decoration ) return true;
            return false;
        }
        ...
        bool isExitDecoration(QPoint& pos)
        {
            if (!enter_decoration) return false;
            QRect isexit (2460,0,close_page.width(),close_page.height());
            if (isexit.contains(pos)) return true;
            return false;
        }

        bool isPointOnDec1(QPoint& pos)
        {
            if (!enter_decoration) return false;
            QRect isondec1 (168,108,rabbithat.width(),rabbithat.height());
            if (isondec1.contains(pos)) return true;
            return false;
        }

        bool isPointOnDec2(QPoint& pos)
        {
            if (!enter_decoration) return false;
            QRect isondec1 (1008,108,doghat.width(),doghat.height());
            if (isondec1.contains(pos)) return true;
            return false;
        }

        bool isPointOnDec3(QPoint& pos)
        {
            if (!enter_decoration) return false;
            QRect isondec1 (1648,138,flower.width(),flower.height());
            if (isondec1.contains(pos)) return true;
            return false;
        }

        bool isPointOnDec4(QPoint& pos)
        {
            if (!enter_decoration) return false;
            QRect isondec1 (168,608,dinosaurhat.width(),dinosaurhat.height());
            if (isondec1.contains(pos)) return true;
            return false;
        }

        bool isPointOnDec5(QPoint& pos)
        {
            if (!enter_decoration) return false;
            QRect isondec1 (908,688,curtain_inshop.width(),curtain_inshop.height());
            if (isondec1.contains(pos)) return true;
            return false;
        }

        bool isPointOnDec6(QPoint& pos)
        {
            if (!enter_decoration) return false;
            QRect isondec1 (1848,688,picture_inshop.width(),picture_inshop.height());
            if (isondec1.contains(pos)) return true;
            return false;
        }
protected:
    void paintEvent(QPaintEvent *event) override
    {
        ...
        if (ended)
        {
            ...
            QString text2 = QString("你成功达到了199好感度，并且只花费了%1天！").arg(Day);
            painter.drawText(pointer_text2,text2);
            QPoint pointer_text3(500,540);
            painter.drawText(pointer_text3,"接下来，你可以继续游戏，尝试买下所有物品或刷到更高的好感度！(好吧你应该已经买下来了)");
        ...
        if (desc_page)
        {
            ...
            QPoint pointer_text2(500,460);
            QString text2 = QString("游戏玩法：每天玩游戏获得金币和好感度，金币可以在商店内购买物品，好感度达到999达成隐藏结局");
            painter.drawText(pointer_text2,text2);
            QPoint pointer_text3(500,540);
            painter.drawText(pointer_text3,"商店中的物品都可以直接增加好感度");
            QPoint pointer_text4(500,620);
            painter.drawText(pointer_text4,"除此之外，装饰品商店中也有很多好东西售卖");
            QPoint pointer_text5(500,700);
            painter.drawText(pointer_text5,"里面的物品不会增加好感度，但是可以让你的房间和猫猫变得更好看");
            QPoint pointer_text6(500,780);
            painter.drawText(pointer_text6,"窗帘可以通过鼠标点击来切换开关状态");
            QPoint pointer_text7(500,860);
            painter.drawText(pointer_text7,"购买帽子后点击小猫就可以切换它戴的帽子");
            QPoint pointer_text8(500,940);
            painter.drawText(pointer_text8,"而对于小游戏，每天只能玩一次，就自己探索有哪些吧");
            return ;
        }
        if (!enter_quiz && !enter_shop && !enter_decoration)
        {
            ...
            if (is_bought_picture)
            {
                QPoint pointer_picture (500,400);
                picture = picture.scaled(400,200);
                painter.drawPixmap(pointer_picture,picture);
            }
...
//            }
        ...
        if (enter_shop)
        {
            ...
            QPoint pointer_close (2460,0);
            painter.drawPixmap(pointer_close,shop_close);
            if (bought_1)
            {
        ...
        {
            painter.drawPixmap(rect(),decoration_shop);

            QPoint pointer_close1 (2460,0);
            painter.drawPixmap(pointer_close1,shop_close);

            QPoint pointer_shop1(168,108);
            painter.drawPixmap(pointer_shop1,rabbithat);
            QPoint pointer_shop2(1008,108);
            painter.drawPixmap(pointer_shop2,doghat);
            QPoint pointer_shop3(1648,138);
            painter.drawPixmap(pointer_shop3,flower_inshop);
            QPoint pointer_shop4(168,608);
            dinosaurhat = dinosaurhat.scaled(800,800);
            painter.drawPixmap(pointer_shop4,dinosaurhat);
            QPoint pointer_shop5(908,688);
            painter.drawPixmap(pointer_shop5,curtain_inshop);
            QPoint pointer_shop6(1848,688);
            picture_inshop = picture_inshop.scaled(400,200);
            painter.drawPixmap(pointer_shop6,picture_inshop);
            QString not_enough = QString("金币不足！");
            QString have_sold_1 = QString("售罄");
            QFont font2 = painter.font();
            font2.setPointSize(60);
            ...
            if (is_bought_picture)
            {
                painter.drawRect(1808,688,280,140);
                font2.setPointSize(20);
                painter.setFont(font2);
                painter.drawText(1908,768,have_sold_1);
            }
        }
        else if (enter_quiz)
        {
            ...
            else if (gameclass == 5)
            {
                ...
                painter.setFont(titleFont);
                painter.setPen(Qt::black);
                painter.drawText(1050, 320, "转盘游戏");
                painter.setFont(QFont("Arial", 30));
    ...
    void mousePressEvent(QMouseEvent *event) override
        {
            if (event->button() == Qt::LeftButton)
            {
                ...
                if (enter_quiz)
                {
                    ...
                    if (gameclass == 1)
                    {
                        if (isPointOnQuizS1(pos1) || isPointOnQuizS2(pos1) || isPointOnQuizS3(pos1)) {
                            bool correct = false;
                            if (isPointOnQuizS1(pos1) && rng == 5) correct = true;
                            else if (isPointOnQuizS2(pos1) && (rng == 1 || rng == 2)) correct = true;
                            else if (isPointOnQuizS3(pos1) && (rng == 3 || rng == 4)) correct = true;
                ...
                {
                    if (isPointOnDec1(pos1) && enter_decoration)
                    {
                        if (coin >= 300)
                        {
                            coin -= 300;
                    ...
                    if (isPointOnDec2(pos1) && enter_decoration)
                    {
                        if (coin >= 300)
                        {
                            coin -= 300;
                    ...
                    if (isPointOnDec3(pos1) && enter_decoration)
                    {
                        if (coin >= 800)
                        {
                            coin -= 800;
                            is_bought_flower = true;
                            update();
                        }
                        else
                        {
                    ...
                    if (isPointOnDec4(pos1) && enter_decoration)
                    {
                        if (coin >= 300)
                        {
                            coin -= 300;
                            is_bought_hat[2] = true;
                            update();
                        }
                        else
                        {
                    ...
                    if (isPointOnDec5(pos1) && enter_decoration)
                    {
                        if (coin >= 500)
                        {
                            coin -= 500;
                            is_bought_curtain = true;
                            update();
                        }
                        else
                        {
                    ...
                    if (isPointOnDec6(pos1) && enter_decoration)
                    {
                        if (coin >= 1000)
                        {
                            coin -= 1000;
                            is_bought_picture = true;
                            update();
                        }
                        else
                        {
                    ...
                    if (isExitDecoration(pos1) && enter_decoration)
                    {
                        enter_decoration = false;
                        update();
                        return;
                    }
                    ...
                    if (isPointOnCatty(pos1) && !enter_quiz && !enter_shop && !enter_decoration)
                    {
                        hat = (hat + 1) % 4;
